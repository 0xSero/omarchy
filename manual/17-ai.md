...
If you have an NVIDIA RTX, Intel Arc Pro or AMD card, _Install > AI > Local AI_ adds a Local AI mark to the bar. It knows the one model that was tested on your exact card, downloads it, runs it in a container only your machine can reach, and opens pi, Claude Code, Codex, OpenCode or any other installed coding agent on it in the folder you pick. It shows each GPU's memory and temperature, and the tokens your agents used. A card that has no tested model yet shows Coming soon, with the list of supported cards. _Remove > AI > Local AI_ deletes the models, their engines and weights, and its settings.
